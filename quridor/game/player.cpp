...
namespace quridor
{
    ...
    void Player::SetBoardPosByCell(Cell& cell)
    {
        ...
        pawn.pos = cell.pos;
        const glm::vec3& cellPos = cell.entity.GetComponent<ECS::TransformComponent>().Position;
        pawn.GetComponent<ECS::TransformComponent>().Position = { cellPos.x, 1.2f, cellPos.z };
    }
            ...
            if (pawnPos.y - 1 >= 0)
            {
                ...
                if (cell.getWallState().south == FREE && playerCell.getWallState().north == FREE)
                {
                    if (cell.isCellOccupied())
                    {
                        ...
                        else if (!board.at(pawnPos.x).at(pawnPos.y - 2).isCellOccupied()
                            && pawnPos.y - 2 >= 0)
                        {
            ...
            if (pawnPos.x - 1 >= 0)
            {
                ...
                if (cell.getWallState().east == FREE && playerCell.getWallState().west == FREE)
                {
                    if (cell.isCellOccupied())
                    {
                        ...
                        else if (!board.at(pawnPos.x - 2).at(pawnPos.y).isCellOccupied()
                            && pawnPos.x - 2 >= 0)
                        {
